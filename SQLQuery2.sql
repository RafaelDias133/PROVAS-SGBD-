...
SELECT c.NomeCurso, p.Nome, p.Sobrenome, d.NomeDepartamento
FROM Cursos c
JOIN Professores p ON c.ProfessorID = p.ProfessorID
JOIN Departamentos d ON c.DepartamentoID = d.DepartamentoID
ORDER BY d.NomeDepartamento DESC, c.NomeCurso ASC;
...
select prof.nome, prof.sobrenome, count(c.cursoID) as contagemCurso
from professores prof
join cursos c on prof.ProfessorID = c.ProfessorID
group by prof.nome, prof.sobrenome
...
Create view ProfessoresIntegralView as
select * 
from professores 
where Contrato = 'Integral'
-- Consulta à View
select * 
from ProfessoresIntegralView 
...
select c.NomeCurso, profView.Nome, profView.Sobrenome 
from ProfessoresIntegralView profView
join cursos c on profView.ProfessorID = c.ProfessorID
